...
namespace kinematics {
	...
	*/
	void Kinematics::stepForward(int collision_check, bool need_recovery_for_collision, bool motion_range_restricted) {
	...
	void Kinematics::stepBackward(int collision_check, bool need_recovery_for_collision, bool motion_range_restricted) {
		// save the current state
		KinematicDiagram prev_state;
		if (need_recovery_for_collision) {
			prev_state = diagram.clone();
		...
		if (motion_range_restricted && min_angle < max_angle) {
			...
			double angle = std::atan2(a.y, a.x);
			double margin = 0.01;
			if (angle < min_angle - 1) angle += M_PI * 2;
			else if (angle > max_angle + margin) angle -= M_PI * 2;
			if (angle < min_angle - margin || angle > max_angle + margin) throw "Out of motion range";
		}
